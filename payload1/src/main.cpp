...
const int LATCH_CLOSED_US = 1500; //needs to be changed
const int LATCH_OPEN_US = 1900; //also needs to be changed
...
void servo_callback(const void *msgin) {
  ...
  if (msg->data == 0) {
    servo.setUs(LATCH_CLOSED_US);
  }
  else if (msg->data == 1) {
    servo.setUs(LATCH_OPEN_US);
  }
}
void setup() {
  Serial.begin(115200);
